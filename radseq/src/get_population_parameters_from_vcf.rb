...
vcf_lines.each do |l|
	unless l[0] == "#"
		...
		line_ary[9..-1].each do |gt_with_info|
			genotypes_at_this_site << gt_with_info.split(":")[0]
		end
...
# Make sure that no genotypes are half-called.
print "Checking for half-called sites..."
genotypes_per_site.each do |gts|
	gts.each do |gt|
		if gt.count(".") == 1
			puts "ERROR: Detected half-called site with genotype #{gt}. These are not supported!"
		end
	end
end
puts " done."
