...
t_blue = 66
t_color = "#bfc1b8"
center_red = 194 # 191 # 147
center_green = 204 # 161
center_blue = 205 # 161
scale_bar_shift = 4
...
# Add labels.
texts << Text.new(0.5*svg_margin,triangle_top_bottom_space+triangle_height+lower_label_vertical_shift,"Helvetica",font_size_l,t_color,"T1","none")
texts << Text.new(1.5*svg_margin+triangle_width,triangle_top_bottom_space+triangle_height+lower_label_vertical_shift,"Helvetica",font_size_l,t_color,"T2","none")
texts << Text.new(central_x,triangle_top_bottom_space+upper_label_vertical_shift,"Helvetica",font_size_l,t_color,"T3","none")
...
t1_lik_diffs.size.times do |x|
	...
	c_red = (center_red + (t1_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_red-center_red) + (t2_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_red-center_red) + (t3_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_red-center_red)).to_i
	c_green = (center_green + (t1_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_green-center_green) + (t2_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_green-center_green) + (t3_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_green-center_green)).to_i
	c_blue = (center_blue + (t1_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_blue-center_blue) + (t2_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_blue-center_blue) + (t3_lik_diffs[x]/max_lik_diff_for_plot.to_f) * (t_blue-center_blue)).to_i
	if [c_red,c_green,c_blue].min >= 0 and [c_red,c_green,c_blue].max <= 255
		...
		puts "Locus outside of plot: #{locus_ids[x]}."
	end
end
...
lines << Line.new(x1,x2,y1,y2,t_color,1,1,"none","none")
texts << Text.new((x1+x2)/2.0,((y1+y2)/2.0)+3,"Helvetica",font_size_s,t_color,"n = #{t1_over_t2}","none")
x1 = central_x+0.5
x2 = svg_margin+triangle_width
y1 = triangle_top_bottom_space+triangle_height+arrow_shift
y2 = triangle_top_bottom_space+triangle_height+arrow_shift
lines << Line.new(x1,x2,y1,y2,t_color,1,1,"none","none")
texts << Text.new((x1+x2)/2.0,((y1+y2)/2.0)+3,"Helvetica",font_size_s,t_color,"n = #{t2_over_t1}","none")
x1 = svg_margin
x2 = central_x-0.5
y1 = triangle_top_bottom_space+triangle_height-arrow_shift
y2 = triangle_top_bottom_space+triangle_height-arrow_shift
lines << Line.new(x1,x2,y1,y2,t_color,1,1,"none","rotate(-60,#{svg_margin},#{triangle_top_bottom_space+triangle_height})")
texts << Text.new((x1+x2)/2.0,((y1+y2)/2.0)-1.3,"Helvetica",font_size_s,t_color,"n = #{t1_over_t3}","rotate(-60,#{svg_margin},#{triangle_top_bottom_space+triangle_height})")
x1 = central_x+0.5
x2 = svg_margin+triangle_width
y1 = triangle_top_bottom_space+triangle_height-arrow_shift
y2 = triangle_top_bottom_space+triangle_height-arrow_shift
lines << Line.new(x1,x2,y1,y2,t_color,1,1,"none","rotate(-60,#{svg_margin},#{triangle_top_bottom_space+triangle_height})")
texts << Text.new((x1+x2)/2.0,((y1+y2)/2.0)-1.3,"Helvetica",font_size_s,t_color,"n = #{t3_over_t1}","rotate(-60,#{svg_margin},#{triangle_top_bottom_space+triangle_height})")
x1 = svg_margin
x2 = central_x-0.5
y1 = triangle_top_bottom_space+triangle_height-arrow_shift
y2 = triangle_top_bottom_space+triangle_height-arrow_shift
lines << Line.new(x1,x2,y1,y2,t_color,1,1,"none","rotate(60,#{svg_margin+triangle_width},#{triangle_top_bottom_space+triangle_height})")
texts << Text.new((x1+x2)/2.0,((y1+y2)/2.0)-1.3,"Helvetica",font_size_s,t_color,"n = #{t3_over_t2}","rotate(60,#{svg_margin+triangle_width},#{triangle_top_bottom_space+triangle_height})")
x1 = central_x+0.5
x2 = svg_margin+triangle_width
y1 = triangle_top_bottom_space+triangle_height-arrow_shift
y2 = triangle_top_bottom_space+triangle_height-arrow_shift
lines << Line.new(x1,x2,y1,y2,t_color,1,1,"none","rotate(60,#{svg_margin+triangle_width},#{triangle_top_bottom_space+triangle_height})")
texts << Text.new((x1+x2)/2.0,((y1+y2)/2.0)-1.3,"Helvetica",font_size_s,t_color,"n = #{t2_over_t3}","rotate(60,#{svg_margin+triangle_width},#{triangle_top_bottom_space+triangle_height})")
